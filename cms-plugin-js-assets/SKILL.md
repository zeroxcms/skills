---
name: cms-plugin-js-assets
description: Ship client-side JavaScript in Workers CMS plugin admin views (cms-plugin-events, cms-plugin-event-action, and any /admin/plugins/* fragment). Use this whenever a plugin admin page needs interactivity — add/remove rows, auto-slug, auto-submit, dynamic form behavior, "the button does nothing", or "my script doesn't run / was stripped" — and BEFORE writing any <script> tag, inline handler, or views/assets/*.js file in a CMS plugin repo. It covers the manifest→approval→SRI pipeline that silently blocks unapproved or modified scripts, the strict-CSP rules (inline JS never runs), the progressive-enhancement pattern that survives them, and the free host behaviors (data-confirm, data-reorder) that often make custom JS unnecessary.
---

# Shipping client-side JS in Workers CMS plugin admin views
...
host's client renderer (`cms/views/assets/client-render.js`) sanitizes the
rendered HTML:
...
- Approved scripts are rewritten in place: the host stamps `integrity`
  (SRI sha384 pinned at approval time), the page `nonce`, and a `?r=<revision>`
  cache-buster. Only then do they execute.

Server side, the host (`cms/src/routes/admin/plugins.ts` `servePluginAsset`)
re-fetches the file from the plugin Worker and **recomputes the hash on every
request**: not approved → 404 "asset not approved"; bytes differ from the
pinned hash → 409 "Asset changed since approval; re-approval required". It
fails closed — an admin never executes bytes they didn't review.
...
The host layout (`cms/views/layout/default.liquid`) already ships CSP-safe
global behaviors any plugin fragment can use declaratively:

- `data-confirm="Are you sure?"` on a `<form>` → confirmation prompt on submit.
- `<table data-reorder="/admin/...">` (+ `data-reorder-handle`,
  `data-reorder-handle-only`, `data-reorder-mode="weight"`, rows with
  `data-id`) → drag-and-drop reorder that POSTs the new order.

If one of these covers the interaction, use it and ship nothing.

## The pipeline (4 steps)

Working example: `cms-plugin-event-action` `views/assets/filter-rows.js`
(add/remove filter rows). Also see `cms-plugin-events` `views/assets/event-new.js`
...
3. **Reference it from the view** (`views/sections/*.liquid`) with the
   CMS-side URL (plugin id, not worker name), usually gated to editors:
   ```liquid
...
Symptoms: buttons that used to appear are gone again, asset URL returns 409.
...
Render the interactive controls server-side but `hidden`; the script unhides
and wires them. No DOM surgery to inject buttons, nothing breaks when the
script is absent, and the no-JS fallback stays honest:
...
- View test: render via the client-view path (`renderView` against
  `x-cms-view-path`) and assert the fragment contains the `data-*` hooks and
  the `/admin/plugins/<id>/assets/<name>.js` script src — and that view-only
  users get neither.
- Asset test: `GET /__plugin/admin/assets/<name>.js` returns 200 with a
  `text/javascript` content-type.
- The approval flow itself is host-side; don't try to simulate it in plugin
  tests.
...
0. **Approval itself fails** ("Could not fetch the asset from the plugin —
   ...
   `/assets/<name>.js` path (it only routed `/__plugin/admin/assets/*`). Add
   the `path.startsWith('/assets/')` branch to the fetch handler (see step 1)
   and redeploy the plugin, then approve again.
1. **Not approved / hash drifted** — open the asset URL directly: 404 "asset
   not approved" or 409 "changed since approval" tells you exactly. Re-approve.
...
3. **Inline JS or `on*` attribute** — sanitizer stripped it; move to an
   external declared asset + `addEventListener`.
4. **Fragment not chrome-wrapped** — only responses with `x-cms-chrome: 1`
   (what `adminView`/`clientViewResponse` set) go through this pipeline;
