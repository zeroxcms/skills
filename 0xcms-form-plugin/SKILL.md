...
- Host ingest/publish contract: either live host — `workers/cms`
  (feature-sliced, ingest at `src/core/db/submission-ingest.ts`) or
  `frameworks/zeroxcms/cms` (monolithic, `src/utils/submission-ingest.ts`).
  Resolve host paths against the repo you are actually editing; see the Source
  authority table in `0xcms-plugin-api`.
...
projections, and public templates. In editors, read exact-language values
(`locExact`) and use the default language only as a placeholder. The general
rule and the full CMS field-name grammar live in `0xcms-plugin-api` →
`references/edit-view.md`; `plugin-form`'s bespoke editor is built on it.
